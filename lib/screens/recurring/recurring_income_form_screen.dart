...
class RecurringIncomeFormScreen extends StatefulWidget {
  final RecurringIncome? initial;
  const RecurringIncomeFormScreen({super.key, this.initial});

  @override
  State<RecurringIncomeFormScreen> createState() =>
      _RecurringIncomeFormScreenState();
}

class _RecurringIncomeFormScreenState extends State<RecurringIncomeFormScreen> {
  ...
  void initState() {
    ...
    _source = r == null
        ? IncomeSources.maas
        : CategoryService.instance.incomeByName(r.source);
  ...
  String _signature() =>
      '${_amountCtrl.text}|${_noteCtrl.text}|${_source.name}|$_dayOfMonth|$_active';
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_isEdit) {
        ...
        await RecurringIncomeRepository.instance.update(updated);
      } else {
        final draft = RecurringIncome(
          userId: user.id!,
          amount: amount,
          source: _source.name,
          note: note,
        ...
        await RecurringIncomeRepository.instance.insert(draft);
      }
      if (!mounted) return;
